...
const NearByScreen = (props) => {
  ...
  const initialMapState = {
    markers,
    region: {
      latitude: 34.0485711902652,
      longitude: -118.08971073524769,
  ...
  useEffect(() => {
    mapAnimation.addListener(({ value }) => {
      ...
      if (index >= initialMapState.markers.length) {
        index = initialMapState.markers.length - 1;
      }
      if (index <= 0) {
        index = 0;
      }
      ...
      const regionTimeout = setTimeout(() => {
        if (mapIndex !== index) {
          mapIndex = index;
          const { coordinate } = initialMapState.markers[index];
  ...
  const interpolations = initialMapState.markers.map((marker, index) => {
        ...
        >
          {initialMapState.markers.map((marker, index) => (
            ...
            >
              ...
              >
                <Text style={{ ...Fonts.Primary16Bold }}>{marker.title}</Text>
