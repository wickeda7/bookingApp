...
const Invoices = (props) => {
  ...
  console.log('date', date, specialistId, storeId, subtotal, tips);
  useEffect(() => {
    const getData = async () => {
  ...
  return (
    <View style={{ flex: 1, backgroundColor: Colors.white }}>
      ...
      >
        <TouchableOpacity style={{ marginHorizontal: Default.fixPadding * 1.5 }} onPress={() => navigation.goBack()}>
          <Ionicons name={isRtl ? 'arrow-forward' : 'arrow-back'} size={30} color={Colors.white} />
        </TouchableOpacity>
        <Text style={Fonts.White18Bold}>{tr('payroll')}</Text>
